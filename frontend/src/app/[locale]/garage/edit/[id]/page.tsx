...
export default function EditCarPage() {
  ...
  const editCar = useGarageStore((s) => s.editCar);
  const loadingStore = useGarageStore((s) => s.loading);
  ...
  useEffect(() => {
    ...
    setHorsepower(car.horsepower != null ? String(car.horsepower) : "");
    setConsumption(car.combined_fuel_consumption != null ? String(car.combined_fuel_consumption) : "");
    setBodyType(car.body_type || "");
    setEquipment(car.equipment ? car.equipment.split(",").filter(Boolean) : []);
    setNotes(car.notes || "");
    setReady(true);
  }, [cars, carId]);
  ...
  return (
    <div className="min-h-screen pt-20 pb-16">
      <div className="max-w-lg mx-auto px-4">
        ...
        <p className="text-sm text-[#6B7280] mb-8">{brand} {model}</p>
        ...
        <div className="space-y-8">
          <section className="space-y-3">
            ...
            <input value={brand} onChange={(e) => setBrand(e.target.value)} placeholder="Marka" className={inputCls} />
            <input value={model} onChange={(e) => setModel(e.target.value)} placeholder="Model" className={inputCls} />
            <input value={variant} onChange={(e) => setVariant(e.target.value)} placeholder={isTr ? "Paket" : "Trim"} className={inputCls} />
            <div className="flex gap-3">
              <input type="number" value={year} onChange={(e) => setYear(e.target.value)} placeholder="Yıl" className={inputCls + " flex-1"} />
              <input type="number" value={price} onChange={(e) => setPrice(e.target.value)} placeholder="Fiyat (₺)" className={inputCls + " flex-[2]"} />
            </div>
          </section>
          ...
          <section className="space-y-3">
            ...
            <input type="number" step="0.1" value={consumption} onChange={(e) => setConsumption(e.target.value)} placeholder="L/100km" className={inputCls} />
          </section>
          ...
          <section>
            ...
            <div className="flex flex-wrap gap-2">
              {EQUIPMENT.map((eq) => {
                ...
                return (
                  <button key={eq.slug} type="button" onClick={() => toggleEquip(eq.slug)}
          ...
          <textarea value={notes} onChange={(e) => setNotes(e.target.value)} rows={2}
            placeholder={isTr ? "Notlar" : "Notes"} className={inputCls + " resize-none"} />
          ...
          <button type="button" onClick={handleSave} disabled={saving}
            ...
            {saving
              ...
              : <><Save className="w-4 h-4" /> {isTr ? "Güncelle" : "Update"}</>}
          </button>
        </div>
      </div>
    </div>
